...
async function LoadPrefixCommand(message: Message, bot: ErineClient) {
...
async function LoadHybridCommand(message: Message, bot: ErineClient) {
    if(message.author.bot) return
        let ctx = new Context(message, bot)
        if(bot._options.guildOnly && !message.guild) return bot.emit('contextError', new Errors.GuildOnly(ctx))
        let prefix: string;
...
const event: Event = {
    ...
    async code(message: Message, bot: ErineClient) {
        LoadHybridCommand(message, bot).catch(e => bot.emit('contextError', new Errors.UnknownCommandError(new Context(message, bot), e)))
        LoadPrefixCommand(message, bot).catch(e => bot.emit('contextError', new Errors.UnknownCommandError(new Context(message, bot), e)))
    }
}
